...
public class TransferFragmentRecyclerViewAdapter extends RecyclerView.Adapter<TransferFragmentRecyclerViewAdapter.ViewHolder> {
    private final List<FileMetaData> files;

    public TransferFragmentRecyclerViewAdapter(@NonNull List<FileMetaData> files) {
        this.files = files;
    }

    @NonNull
    @Override
    public TransferFragmentRecyclerViewAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        var view = LayoutInflater.from(parent.getContext()).inflate(R.layout.heading_detail_row_recycler_view, parent,false);
        return new TransferFragmentRecyclerViewAdapter.ViewHolder(view);
    }
